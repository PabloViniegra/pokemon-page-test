import type { PokemonResultList, PokemonDetail, PokemonSpeciesInfo, AbilityDetail } from '../types/pokemon'

const POKEMON_API_BASE = 'https://pokeapi.co/api/v2'

export async function getPokemonsList(limit = 20, offset = 0): Promise<PokemonResultList> {
    const url = `${POKEMON_API_BASE}/pokemon?limit=${limit}&offset=${offset}`
    const response = await fetch(url)

    if (!response.ok) {
        throw new Error(`Failed to fetch pokemons: ${response.status} ${response.statusText}`)
    }
...
export async function getPokemonDetail(nameOrId: string | number): Promise<PokemonDetail> {
    ...
    if (!response.ok) {
        throw new Error(`Failed to fetch pokemon detail: ${response.status} ${response.statusText}`)
    }

    return await response.json() as PokemonDetail
}

export async function getPokemonSpecies(url: string): Promise<PokemonSpeciesInfo> {
    const response = await fetch(url)
...
export async function getPokemonsByType(type: string): Promise<PokemonResultList> {
    const url = `${POKEMON_API_BASE}/type/${type}`
    const response = await fetch(url)

    if (!response.ok) {
        throw new Error(`Failed to fetch pokemons by type: ${response.status} ${response.statusText}`)
    }

    const data = await response.json() as { pokemon: { pokemon: { name: string; url: string } }[] }
    return {
...
export async function getAbilityDetail(name: string): Promise<AbilityDetail> {
    ...
    if (!response.ok) {
        throw new Error(`Failed to fetch ability: ${response.status} ${response.statusText}`)
    }
