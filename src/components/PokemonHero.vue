...
const mainImage = computed(() => getPokemonImageUrl(props.pokemon.id, props.showShiny))

const description = computed(() => {
    if (!props.species) return ''
    const entry = props.species.flavor_text_entries.find(e => e.language.name === 'en')
...
<template>
  <section class="relative overflow-hidden" :style="{ background: `linear-gradient(135deg, ${primaryTypeColor}15 0%, ${primaryTypeColor}05 50%, transparent 100%)` }">
    <div class="absolute right-0 top-0 w-96 h-96 opacity-5 transform translate-x-1/4 -translate-y-1/4">
      ...
      <div class="grid md:grid-cols-2 gap-12 items-center">
        <div class="relative flex flex-col items-center">
          <div class="relative w-72 h-72 md:w-96 md:h-96">
            <div class="absolute inset-0 rounded-full opacity-20 blur-3xl" :style="{ backgroundColor: primaryTypeColor }"></div>
            <img
              :src="mainImage"
              :alt="pokemon.name"
          ...
          <button
            ...
            :class="{ 'border-yellow-400 text-yellow-600 bg-yellow-50': showShiny }"
          >
            <span>✨</span>
        ...
        <div class="space-y-6">
          <div>
            <p class="text-gray-400 font-bold text-sm mb-1">#{{ String(pokemon.id).padStart(4, '0') }}</p>
            <div class="flex items-center gap-3">
              <h1 class="text-5xl md:text-6xl font-black capitalize tracking-tight text-gray-900" style="font-family: 'Fredoka', sans-serif;">
                {{ pokemon.name }}
              </h1>
              <button
            ...
            <p v-if="genus" class="text-gray-500 font-medium mt-2">{{ genus }}</p>
          </div>
          ...
          <div class="flex flex-wrap gap-x-6 gap-y-2 text-sm">
            <div class="flex items-center gap-2">
              <span class="text-gray-400 font-bold">Height</span>
              <span class="font-black text-gray-900 font-mono">{{ pokemon.height / 10 }}<span class="text-gray-400 font-medium ml-0.5">m</span></span>
            </div>
            <div class="flex items-center gap-2">
              <span class="text-gray-400 font-bold">Weight</span>
              <span class="font-black text-gray-900 font-mono">{{ pokemon.weight / 10 }}<span class="text-gray-400 font-medium ml-0.5">kg</span></span>
            </div>
            <div class="flex items-center gap-2">
              <span class="text-gray-400 font-bold">Base XP</span>
              <span class="font-black text-gray-900 font-mono">{{ pokemon.base_experience }}</span>
            </div>
          </div>
        </div>
      </div>
