    ...
    <div class="flex flex-wrap gap-3">
      ...
      >
        <div class="flex items-center gap-2">
          <span class="text-gray-900 font-bold capitalize text-sm">{{ ability.ability.name.replace('-', ' ') }}</span>
          <span v-if="ability.is_hidden" class="text-[10px] bg-gray-200 text-gray-500 px-2 py-0.5 rounded-full font-bold uppercase" title="This ability is only visible in-game with specific items or conditions">Hidden</span>
        </div>
        <p v-if="getShortEffect(index)" class="text-gray-500 text-xs leading-relaxed">{{ getShortEffect(index) }}</p>
      </div>
    </div>
  </section>
</template>
