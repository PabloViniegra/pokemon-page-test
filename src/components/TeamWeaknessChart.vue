...
const sortedAnalysis = computed(() => {
    return [...analysis.value].sort((a, b) => b.averageEffectiveness - a.averageEffectiveness)
})
...
const vulnerabilitySummary = computed(() => {
    const critical = analysis.value.filter(a => a.weakCount === props.teamTypes.length && props.teamTypes.length > 0)
    const major = analysis.value.filter(a => a.weakCount >= 3 && a.weakCount < props.teamTypes.length)
    const resistant = analysis.value.filter(a => a.resistantCount + a.immuneCount === props.teamTypes.length && props.teamTypes.length > 0)
  ...
  <div class="space-y-6">
    ...
    <div v-if="teamTypes.length > 0" class="space-y-3">
      ...
      >
        ...
        <div>
          <p class="font-bold text-red-700 text-sm">Critical vulnerability detected!</p>
      ...
      >
        <div class="w-8 h-8 rounded-full bg-orange-500 flex items-center justify-center text-white shrink-0">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
        </div>
      ...
      >
        ...
        <div>
          ...
          <p class="text-green-600 text-xs">Your whole team resists {{ item.type }}!</p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border-2 border-gray-100 p-5">
      <h3 class="text-lg font-bold text-gray-900 mb-4">Type Matchup Analysis</h3>

      <div v-if="teamTypes.length === 0" class="text-center py-8 text-gray-400">
        <p class="font-medium">Add Pokémon to your team to see weakness analysis</p>
      </div>

      <div v-else class="space-y-3">
        <div
          v-for="item in sortedAnalysis"
          :key="item.type"
          class="group"
        >
          <div class="flex items-center gap-3">
            ...
            <div
              ...
              :style="{ backgroundColor: TYPE_COLORS[item.type]?.color ?? '#999' }"
            >
              <span class="text-xs font-bold text-white capitalize">{{ item.type }}</span>
            </div>

            <!-- Bar -->
            <div class="flex-1 h-8 bg-gray-100 rounded-xl overflow-hidden relative">
              <div
                class="h-full rounded-xl transition-all duration-500 flex items-center px-2"
                :class="getBarBgClass(item.averageEffectiveness)"
                :style="{ width: `${Math.min((item.averageEffectiveness / maxAvg) * 100, 100)}%` }"
              >
            ...
            <div class="shrink-0 flex gap-2 text-[10px] font-semibold">
              <span v-if="item.weakCount > 0" class="text-red-500">{{ item.weakCount }} weak</span>
              <span v-if="item.resistantCount > 0" class="text-green-500">{{ item.resistantCount }} resist</span>
              <span v-if="item.immuneCount > 0" class="text-gray-500">{{ item.immuneCount }} immune</span>
            </div>
          </div>
        </div>
      </div>
