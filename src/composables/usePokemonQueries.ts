import { useInfiniteQuery, useQueries, useQuery, useQueryClient } from '@tanstack/vue-query'
import { getPokemonsList, getPokemonDetail, getPokemonSpecies, getPokemonsByType } from '../helpers/pokemon-api'
...
export const pokemonKeys = {
    ...
    speciesDetail: (id: string | number) => [...pokemonKeys.species(), id] as const,
}
...
export function usePokemonSearchQuery(searchTerm: MaybeRefOrGetter<string>) {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (!term) return { count: 0, next: null, previous: null, results: [] } as PokemonResultList
            ...
            try {
                ...
                return {
                    ...
                    next: null,
                    previous: null,
                    results: [{ name: detail.name, url: `https://pokeapi.co/api/v2/pokemon/${detail.id}/` }],
...
export function usePokemonSpeciesQuery(id: MaybeRefOrGetter<string | number>, url?: MaybeRefOrGetter<string | undefined>) {
    return useQuery({
